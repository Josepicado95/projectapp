...
### Sub-proyecto 3 — Dashboard / misiones móvil ✅ Completo (2026-07-07, PRs #9-#13)
...
- [x] **Ronda C2a** — CRUD de aventuras (crear con título+paisaje, editar, borrar con
      ...
      registro de avance.
- [x] **Ronda C2b** — CRUD de misiones (una sola pantalla compartida para crear/editar/borrar,
      sin `fetch` al montar). Implementada y verificada en celular (2026-07-07) — ver
      registro de avance.

**Con esto, el Sub-proyecto 3 completo (Rondas A, B, C1, C2a, C2b) queda cerrado — solo
queda pendiente la Ronda A.5 (port del cielo animado), que se desacopló desde el
brainstorming inicial por su propio riesgo técnico.**
...
**Spec/plan Ronda C2b:** `apps/mobile/docs/superpowers/specs/2026-07-07-mobile-mission-crud-design.md`,
`apps/mobile/docs/superpowers/plans/2026-07-07-mobile-mission-crud.md`
...
### Sub-proyecto 4 — Cerrar la migración a móvil ⏳ Sin empezar

Con el Sub-proyecto 3 completo, Jose revisó el estado general de `apps/mobile` frente a
`apps/web` y definió 3 brechas restantes, en este orden de prioridad (2026-07-07):

- [ ] **1. Registro de cuenta nueva en móvil** (prioridad más alta) — hoy `apps/mobile` solo
      tiene login; un usuario nuevo debe registrarse primero desde `apps/web`. Objetivo:
      que todas las funcionalidades de web se puedan hacer desde móvil sin depender de web.
      Sin spec todavía.
- [ ] **2. Saldar la deuda técnica conocida** — en particular el timeout de `fetch` faltante
      en `lib/api.ts` (confirmado en vivo cinco veces durante el Sub-proyecto 3), que es la
      de mayor impacto real percibido. Ver la lista completa en `CLAUDE.md`, sección de
      deuda técnica.
- [ ] **3. Ronda A.5 — pulir el cielo animado o rediseñar la interfaz móvil** (prioridad más
      baja) — port de `sky-engine.ts` a React Native, o alternativamente una revisión de
      diseño visual más amplia; a decidir cuál cuando llegue el turno. Riesgo técnico ya
      identificado: las texturas de `sky-engine.ts` usan Canvas 2D del DOM, inexistente en RN.
...
- 2026-07-07 — Brainstorming + Ronda C2b (CRUD de misiones móvil) completa de punta a punta, cerrando el Sub-proyecto 3 (Rondas A, B, C1, C2a, C2b). Decisión de arquitectura: una sola pantalla compartida `adventures/[id]/missions/[missionId].tsx` para crear/editar/borrar (con `missionId === "new"` como bandera de modo), a diferencia de C2a que usó dos pantallas separadas — justificado porque el formulario de misión es más simple (solo título+dificultad) y casi idéntico entre crear/editar, igual que el propio modal de web. Detalle clave: no existe `GET /api/mobile/missions/:id` en el backend, así que la pantalla de edición no hace ningún `fetch` — Adventure Detail le pasa el título/dificultad actuales como parámetros de navegación (ya los tiene en memoria), evitando una llamada de red innecesaria. Task 1: la pantalla compartida. Task 2: botón "+ Agregar misión" + separación del toggle existente y un nuevo botón "Editar" por fila (dos `Pressable` hermanos en vez de uno solo, para no chocar con el toggle de completado). Revisión final del branch: `Ready to merge: Yes`, sin hallazgos Críticos/Importantes; 1 Menor real y plan-mandated (guardar+borrar casi simultáneo dispara dos `router.back()`, aterrizando en el Dashboard en vez de Adventure Detail) — Jose decidió arreglarlo, cerrado con una traba `useRef` (`goBackOnce()`) de 2 líneas. Verificación manual en celular: crear→editar (precargado sin spinner)→borrar confirmado, toggle y "Editar" no se pisan entre sí. Durante la verificación se topó una falsa alarma (el servidor de desarrollo, corriendo sin reiniciar desde hacía muchas horas, dejó de reconocer una ruta real — confirmado con `curl` antes/después de reiniciar que era un problema del proceso, no del código) y quedó una misión de prueba de esa investigación que se limpió después; aparte de eso, el caso de servidor caído volvió a topar con la misma deuda de timeout de `fetch` (quinta vez, cinco rondas distintas). Mergeado a `main` vía PR #13.
- 2026-07-07 — Cierre de sesión: con el Sub-proyecto 3 (dashboard/misiones móvil) completo, Jose preguntó si la migración a móvil podía darse por terminada. Se revisó la paridad funcional contra `apps/web` (tabla completa: login, dashboard, misiones, check-in, progreso — todo cubierto) y se identificaron 3 brechas restantes: registro de cuenta en móvil (solo hay login hoy), deuda técnica conocida (sobre todo el timeout de `fetch`), y el cielo animado/diseño visual (Ronda A.5). Jose definió el orden de prioridad: 1) registro de cuenta, 2) deuda técnica, 3) cielo/rediseño — ver Sub-proyecto 4 arriba. Decidió no arrancar la Ronda D en esta misma sesión (ya muy larga); queda anotado para la próxima.
