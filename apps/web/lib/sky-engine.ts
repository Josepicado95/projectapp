 ...
 *
 * Adapted from sky-engine.js design file (v2 — wind, snow, aurora, cliff, flowers).
...
const MOMENTS: Record<SkyMomentKey, {
  sky: string[];
  horizon: number;
  ...
  water: { top: string; deep: string; reflect: string } | null;
  ground: string | null;
  layers: Layer[];
  cliff: { side: number; color: string; pineColor: string; grass: string; topY: number; tipX: number; outerX: number; pines: number } | null;
  aurora: { bands: Array<{ y: number; w: number; h: number; c1: string; c2: string; spd: number; op: number }> } | null;
  stars: number; fireflies: number; gulls: number; shootingStars: boolean; motes: boolean; snow: boolean;
  leaves: { colors: string[]; count: number; size: number } | null;
  grass: { color: string; count: number; y: number; h: number; opacity: number } | null;
  flowers: { colors: string[]; count: number; size: number; yTop: number; yBot: number } | null;
  butterflies: number;
}> = {
  dawn: {
    ...
    cliff: null,
    aurora: null,
    stars:0, fireflies:0, gulls:3, shootingStars:false, motes:false, snow:false,
  ...
  noon: {
    ...
    ground: '#6EA430',
    layers: [
      { type:'hill', color:'#B2D46C', baseY:0.0,   amp:0.08, jag:0.0, speed:0.007, opacity:1, snow:null, snowDepth:0, snowOpacity:0 },
    ...
    cliff: null,
    aurora: null,
    stars:0, fireflies:0, gulls:0, shootingStars:false, motes:true, snow:false,
    leaves: null,
  ...
  dusk: {
    ...
    water: { top:'#F0C070', deep:'#6E5478', reflect:'#FFE2A2' },
    ground: null,
    layers: [] as typeof MOMENTS['dawn']['layers'],
  ...
  night: {
    ...
    ground: '#0A1424',
    layers: [
      { type:'mtn',   color:'#20304E', baseY:0.08,  amp:0.32, jag:0.11, speed:0.005, opacity:1, snow:'#C6D4E6', snowDepth:0.05,  snowOpacity:0.92 },
      { type:'mtn',   color:'#152038', baseY:-0.05, amp:0.26, jag:0.10, speed:0.009, opacity:1, snow:'#9EB2CC', snowDepth:0.045, snowOpacity:0.82 },
      { type:'trees', color:'#070E1C', baseY:-0.42, amp:0.0,  jag:0.0,  speed:0.030, opacity:1, snow:null, snowDepth:0, snowOpacity:0 },
    ],
    cliff: null,
    aurora: { bands: [
    ...
    stars:1.0, fireflies:0, gulls:0, shootingStars:true, motes:false, snow:true,
...
function create(canvas: HTMLCanvasElement, opts?: SkyEngineOpts): SkyEngineCtrl {
  ...
  // Global wind — ebbing gusts (~0.05 .. ~1.6), shared by everything
  ...
  function build(key: SkyMomentKey) {
    dyn.traverse((o) => {
      ...
      if (m.material) { const ms=Array.isArray(m.material)?m.material:[m.material]; ms.forEach((x:THREE.Material)=>x.dispose()); }
    });
    ...
    if (M.water) {
      const wMat = new THREE.ShaderMaterial({
        ...
        uniforms: {
          uTime:{value:0}, uTop:{value:new THREE.Color(M.water.top)},
          uDeep:{value:new THREE.Color(M.water.deep)}, uRef:{value:new THREE.Color(M.water.reflect)},
          uSunX:{value:M.celestial.x}, uHorizon:{value:M.horizon},
        },
      ...
      const wMesh=new THREE.Mesh(new THREE.PlaneGeometry(W*4,2.4),wMat);
      wMesh.position.y=M.horizon-1.2; wMesh.renderOrder=-90; dyn.add(wMesh);
      updaters.push(t=>{ wMat.uniforms.uTime.value=t; });
    } else if (M.ground) {
      ...
      gMesh.position.y=M.horizon-1.2; gMesh.renderOrder=-90; dyn.add(gMesh);
    }

    // Aurora (rendered behind celestial)
    if (M.aurora) addAurora(M.aurora.bands);

    // Celestial
    const C = M.celestial;
    const celGroup=new THREE.Group(); celGroup.position.set(C.x*aspect, C.y, 0);
    ...
    if (CL.count > 0) {
      ...
      for(let i=0;i<CL.count;i++){
        ...
        const x0=(Math.random()*2-1)*W, y0=CL.yMin+Math.random()*(CL.yMax-CL.yMin);
    ...
    // Near parallax (in front of clouds)
    ...
    function addSilhouette(l: Layer, baseRender: number) {
      const segs=320; const span=W*4;
      const freqs=[0.5,1,2,3,5]; const amps=[1.0,0.6,0.28,0.13,0.05];
      ...
      const mesh=new THREE.Mesh(geo,mat); mesh.renderOrder=baseRender+Math.round(l.speed*100);
      const grp=new THREE.Group(); grp.add(mesh); dyn.add(grp);

      if(l.snow){
        const cap=l.snowDepth||0.06;
        ...
        const sgeo=new THREE.ShapeGeometry(s2);
        const smat=new THREE.MeshBasicMaterial({color:new THREE.Color(l.snow),transparent:true,opacity:l.snowOpacity||0.9,depthTest:false,depthWrite:false});
        const smesh=new THREE.Mesh(sgeo,smat); smesh.renderOrder=mesh.renderOrder+0.5; grp.add(smesh);
      }
    ...
    function addPineRow(grp: THREE.Group, l: Layer, baseRender: number) {
      ...
      for(let i=0;i<=count;i++){
        ...
        const tiers:[[number,number,number],[number,number,number],[number,number,number]]=[
          [baseY+th,       baseY+th+h*0.46, w],
          [baseY+h*0.30,   baseY+h*0.74,    w*0.74],
          [baseY+h*0.58,   baseY+h*1.02,    w*0.50],
        ];
        tiers.forEach(([by,ay,hw])=>{
          pushTri(x-hw,by,x+hw,by,x,ay,sy(by),sy(by),sy(ay),ph);
        });
      }
      ...
      grp.add(pmesh);
      updaters.push(t=>{ m.uniforms.uTime.value=t; m.uniforms.uWind.value=windAt(t); });
    }

    function addWindStreaks() {
      const N=26;
      interface Streak{x:number;y:number;len:number;spd:number;a:number;yb:number}
      const data:Streak[]=[];
      const yRange=()=>M.horizon+0.06+Math.random()*(1-M.horizon)*0.82;
      for(let i=0;i<N;i++) data.push({x:(Math.random()*2-1)*aspect*1.4,y:yRange(),len:0.14+Math.random()*0.42,spd:0.5+Math.random()*0.8,a:0.05+Math.random()*0.09,yb:Math.random()*TWO_PI});
      const pos=new Float32Array(N*6),al=new Float32Array(N*2);
      const g=new THREE.BufferGeometry();
      ...
      const seg=new THREE.LineSegments(g,m); seg.renderOrder=-58; dyn.add(seg);
      const attr=g.attributes.position as THREE.BufferAttribute, aattr=g.attributes.aA as THREE.BufferAttribute;
      updaters.push((t,dt)=>{
        const wind=windAt(t);
        data.forEach((s,i)=>{
          ...
          attr.setXYZ(i*2,s.x,yy,0);
          attr.setXYZ(i*2+1,s.x-s.len*(0.5+wind*0.6),yy-0.006,0);
          aattr.setX(i*2,s.a*Math.min(1,wind));
          aattr.setX(i*2+1,0);
        });
        attr.needsUpdate=true; aattr.needsUpdate=true;
      });
    }

    function addLeaves(cfg: {colors:readonly string[];count:number;size:number}) {
      const N=cfg.count||18; const cols=cfg.colors;
      ...
      for(let i=0;i<N;i++){
        const x=(Math.random()*2-1)*aspect*1.3, y=M.horizon-0.02+Math.random()*(1-M.horizon)*0.9;
        pos[i*3]=x;pos[i*3+1]=y;pos[i*3+2]=0;ph[i]=Math.random()*TWO_PI;
        const col=new THREE.Color(cols[i%cols.length]);cc[i*3]=col.r;cc[i*3+1]=col.g;cc[i*3+2]=col.b;
        sz[i]=(cfg.size||5)*(0.6+Math.random()*0.9);
      ...
      const pts=new THREE.Points(g,m); pts.renderOrder=-36; dyn.add(pts);
      const attr=g.attributes.position as THREE.BufferAttribute;
      updaters.push((t,dt)=>{
        m.uniforms.uTime.value=t; const wind=windAt(t);
        data.forEach((f,i)=>{
          let x=attr.getX(i)-f.spd*(0.35+wind)*dt*(isStatic?0.4:1);
    ...
    function addGrass(cfg:{color:string;count:number;y:number;h:number;opacity:number}) {
      const count=cfg.count||120;
      const positions:number[]=[],sway:number[]=[],phase:number[]=[];
      const yBase=cfg.y!=null?cfg.y:(M.horizon-0.45);
      const H=cfg.h||0.16;
      for(let i=0;i<count;i++){
        const x=-aspect*1.2+(i/count)*aspect*2.4+(Math.random()-0.5)*0.02;
        const h=H*(0.5+Math.random()*0.9), w=h*0.05+0.004;
        const by=yBase+(Math.random()-0.5)*0.05, lean=(Math.random()-0.5)*w*2.5;
      ...
      const m=new THREE.ShaderMaterial({
        ...
        uniforms:{uTime:{value:0},uWind:{value:0},uColor:{value:new THREE.Color(cfg.color)},uOp:{value:cfg.opacity!=null?cfg.opacity:1}},
      ...
      const mesh=new THREE.Mesh(g,m); mesh.renderOrder=-35; dyn.add(mesh);
      updaters.push(t=>{ m.uniforms.uTime.value=t; m.uniforms.uWind.value=windAt(t); });
    }

    function addAurora(bands: readonly {y:number;w:number;h:number;c1:string;c2:string;spd:number;op:number}[]) {
      bands.forEach((b,bi)=>{
    ...
    function addSnow() {
      ...
      for(let i=0;i<N;i++){
        const x=(Math.random()*2-1)*aspect*1.3, y=M.horizon+Math.random()*(1-M.horizon+0.2);
      ...
      const pts=new THREE.Points(g,m); pts.renderOrder=-34; dyn.add(pts);
      const attr=g.attributes.position as THREE.BufferAttribute;
      updaters.push((t,dt)=>{
        m.uniforms.uTime.value=t; const wind=windAt(t);
        data.forEach((f,i)=>{
          ...
          attr.setXYZ(i,f.x+Math.sin(t*0.8+f.phase)*f.amp,f.y,0);
        });
        attr.needsUpdate=true;
      });
    }

    function addCliff(cfg:{side:number;color:string;pineColor:string;grass:string;topY:number;tipX:number;outerX:number;pines:number}) {
      const side=cfg.side||1;
      const outerX=(cfg.outerX||1.35)*aspect*side;
      const tipX=(cfg.tipX!=null?cfg.tipX:0.03)*aspect*side;
      const topY=cfg.topY!=null?cfg.topY:(M.horizon+0.14);
      const N=48; const topEdge:Array<[number,number]>=[];
      const shape=new THREE.Shape();
      ...
      const mesh=new THREE.Mesh(new THREE.ShapeGeometry(shape),new THREE.MeshBasicMaterial({color:new THREE.Color(cfg.color),depthTest:false,depthWrite:false}));
      ...
      if(cfg.grass){
        ...
        const smesh=new THREE.Mesh(new THREE.ShapeGeometry(s2),new THREE.MeshBasicMaterial({color:new THREE.Color(cfg.grass),depthTest:false,depthWrite:false}));
        smesh.renderOrder=-32.5; dyn.add(smesh);
      }
      const pineCount=cfg.pines!=null?cfg.pines:3;
      const positions:number[]=[],sway:number[]=[],phase:number[]=[];
      for(let p=0;p<pineCount;p++){
        const tt=0.32+(p/Math.max(1,pineCount))*0.5+Math.random()*0.05;
        ...
        const sy=(yy:number)=>Math.min(1,Math.max(0,(yy-baseY)/h));
        [[baseY,baseY+h*0.5,w],[baseY+h*0.28,baseY+h*0.78,w*0.72],[baseY+h*0.56,baseY+h*1.04,w*0.48]]
      ...
      if(positions.length){
        ...
        pg.setAttribute('aSway',new THREE.Float32BufferAttribute(sway,1));
        pg.setAttribute('aPhase',new THREE.Float32BufferAttribute(phase,1));
        const pm=new THREE.ShaderMaterial({
          depthTest:false,depthWrite:false,side:THREE.DoubleSide,
          uniforms:{uTime:{value:0},uWind:{value:0},uColor:{value:new THREE.Color(cfg.pineColor||cfg.color)}},
        ...
        const pmesh=new THREE.Mesh(pg,pm); pmesh.renderOrder=-32; dyn.add(pmesh);
        updaters.push(t=>{ pm.uniforms.uTime.value=t; pm.uniforms.uWind.value=windAt(t); });
      }
    }

    function addFlowers(cfg:{colors:readonly string[];count:number;size:number;yTop:number;yBot:number}) {
      const N=cfg.count||60, cols=cfg.colors;
      const pos=new Float32Array(N*3),cc=new Float32Array(N*3),sz=new Float32Array(N),ph=new Float32Array(N);
      const yTop=cfg.yTop!=null?cfg.yTop:(M.horizon-0.1), yBot=cfg.yBot!=null?cfg.yBot:-0.86;
      for(let i=0;i<N;i++){
        const x=(Math.random()*2-1)*aspect*1.15, y=yBot+Math.random()*(yTop-yBot);
        pos[i*3]=x;pos[i*3+1]=y;pos[i*3+2]=0;
        const col=new THREE.Color(cols[i%cols.length]);cc[i*3]=col.r;cc[i*3+1]=col.g;cc[i*3+2]=col.b;
        sz[i]=(cfg.size||6)*(0.55+Math.random()*0.7)*(0.55+(y-yBot)/(yTop-yBot)); ph[i]=Math.random()*TWO_PI;
      }
      const g=new THREE.BufferGeometry();
      g.setAttribute('position',new THREE.Float32BufferAttribute(pos,3));
      g.setAttribute('aCol',new THREE.Float32BufferAttribute(cc,3));
    ...
    function addButterflies(n: number) {
      ...
      for(let i=0;i<n;i++){
        ...
        const g=new THREE.BufferGeometry();
        g.setAttribute('position',new THREE.Float32BufferAttribute([0,0,0,-0.028,0.02,0,-0.028,-0.02,0, 0,0,0,0.028,0.02,0,0.028,-0.02,0],3));
        g.setIndex([0,1,2,3,4,5]);
        const mat=new THREE.MeshBasicMaterial({color:col,transparent:true,opacity:0.92,depthTest:false,depthWrite:false,side:THREE.DoubleSide});
        const mesh=new THREE.Mesh(g,mat);
        const ox=(Math.random()*2-1)*aspect, oy=M.horizon-0.05+Math.random()*0.4;
    ...
    function addFireflies() {
      const N=50; const data:Array<{ox:number;oy:number;spd:number;amp:number;phase:number}>=[];
      const pos=new Float32Array(N*3),ph=new Float32Array(N);
      for(let i=0;i<N;i++){
        const x=(Math.random()*2-1)*aspect*1.1, y=M.horizon-0.05+Math.random()*0.5;
        pos[i*3]=x;pos[i*3+1]=y;pos[i*3+2]=0;ph[i]=Math.random()*TWO_PI;
      ...
      updaters.push(t=>{
        ...
        data.forEach((f,i)=>{ attr.setXYZ(i,f.ox+Math.sin(t*f.spd+f.phase)*f.amp*1.4,f.oy+Math.sin(t*f.spd*0.6+f.phase+1.2)*f.amp,0); });
        attr.needsUpdate=true;
      });
    }
    ...
    function addGulls(n: number) {
      ...
      for(let i=0;i<n;i++){
        ...
        const s=0.7+Math.random()*0.8; line.scale.setScalar(s); dyn.add(line);
    ...
    function addMotes() {
      ...
      for(let i=0;i<N;i++){
        const x=(Math.random()*2-1)*aspect, y=M.horizon-0.05+Math.random()*0.45;
        pos[i*3]=x;pos[i*3+1]=y;pos[i*3+2]=0;ph[i]=Math.random()*TWO_PI;
      ...
      updaters.push(t=>{
        m.uniforms.uTime.value=t;
        data.forEach((f,i)=>{ attr.setXYZ(i,f.ox+Math.sin(t*f.spd+f.phase)*f.amp,f.oy+Math.cos(t*f.spd*0.7+f.phase)*f.amp*0.7,0); });
        attr.needsUpdate=true;
      });
    }
